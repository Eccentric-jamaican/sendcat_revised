...
type SearchCacheResult = { itemIds: Id<"items">[] } | null;
...
function cacheKeyFor(params: { source: string; query: string; filters?: unknown }) {
  const raw = `${params.source}::${params.query}::${JSON.stringify(params.filters ?? {})}`.toLowerCase();
